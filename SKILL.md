...
description: Review SQL queries, schema definitions, and migrations against PostgreSQL best practices. Use this skill when reviewing database-related code — SQL files, migrations, ORM schema definitions, or query changes in a diff — to find performance, security, and correctness issues.
license: MIT
metadata:
  version: "1.0.0"
...
5. **Report findings**, ordered by the rule's impact level. For each finding include:
   ...
   If nothing violates the rules, say so explicitly rather than inventing low-value findings.
